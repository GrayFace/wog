...
local function debug_hook(event, line)
  ...
  elseif event == "line" then
    ...
    if status and res and res ~= 'stack' then
      if abort == nil and res == "exit" then done(); return end
      abort = res
...
local function debugger_loop(sev, svars, sfile, sline)
  ...
  local loaded = {}
  for k in pairs(package_loaded) do loaded[k] = true end
  ...
  while true do
    ...
    elseif command == "LOAD" then
      ...
      if abort == nil then -- no LOAD/RELOAD allowed inside start()
        ...
        for k in pairs(package_loaded) do
          if not loaded[k] then package_loaded[k] = nil end
        end
...
package_loaded.mobdebug = mobdebug
