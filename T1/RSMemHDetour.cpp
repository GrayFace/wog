...
static int GetHookSize(lua_State *L)
{
	...
	while (n < 5)
	{
		...
		n += a.len;
		jumps = jumps || (a.opcode >= 0x70 && a.opcode <= 0x7F  // conditional jump near
			|| a.opcode == 0x0F && a.opcode2 >= 0x80 && a.opcode2 <= 0x8F  // conditional jump far
...
{
	hde32s a;
	hde32_disasm((void*)ToDWORD(L, 1), &a);
	lua_settop(L, 0);
